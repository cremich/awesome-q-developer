...
- Added a blog post resource to the Game Development section:

  - "Game building fun with a 12-year-old and the Amazon Q Developer CLI" - A father-son weekend project creating two games (PacNibbles and a platformer) using Amazon Q Developer CLI and Pygame, demonstrating how AI coding assistants can make game development accessible even for children.

- Previously added a blog post resource to the Developer Productivity section:

  - "Turbo Charge the Amazon Q Developer CLI with MCP Servers" - A comprehensive guide on integrating custom MCP servers with Amazon Q Developer CLI, demonstrating how to enhance its capabilities with domain-specific knowledge and tools using a custom Amazon IVS MCP server example.

- Previously added a blog post resource to the Developer Productivity section:

  - "Crushing the Command Line with Amazon Q: Building a Medium to DEV.to Converter" - A detailed walkthrough of building a command-line tool that automatically converts Medium blog posts to DEV.to-compatible markdown format and optionally publishes them directly to DEV.to as draft posts, showcasing how Amazon Q Developer CLI can be used to create practical automation tools.

- Previously added a blog post resource to the Developer Productivity section:

  - "Configuring Model Context Protocol (MCP) with Amazon Q CLI" - A comprehensive guide on setting up and configuring MCP servers with Amazon Q CLI, including creating the mcp.json configuration file, adding multiple servers, managing trust settings, and security considerations.

- Previously added a blog post resource to the Developer Productivity section:

  - "Q-bits: Easy video editing with Q Developer command line" - A blog post that demonstrates how Amazon Q Developer CLI can simplify working with complex command-line tools like ffmpeg for video editing tasks, helping you combine video segments, trim content, and optimize file sizes through natural language prompts.

- Previously added a blog post resource to the Operations section:

  - "GitHub Actions + Amazon Q: Modernize ECS Deployments" - A blog post that provides a step-by-step guide to automating ECS deployments using GitHub Actions and Amazon Q Developer, including blue/green deployment patterns, CloudWatch monitoring, and infrastructure as code with Terraform.

- Previously added a blog post resource to the Developer Productivity section:

  - "Automate Public Disclosure Using Amazon Q Developer CLI" - A blog post that demonstrates how to use Amazon Q Developer CLI with natural language prompts to automate complex workflows for handling public disclosure requests in Amazon Connect contact centers, eliminating the need for manual work or custom scripting.

- Previously added a blog post resource to the Operations section:

  - "Enhancing Network Security with EC2 Security Groups with Amazon Q Developer" - A blog post that demonstrates how Amazon Q Developer streamlines AWS cloud security by providing expert guidance for EC2 Security Groups, helping teams automate and maintain robust security measures through automated rule creation and compliance monitoring.

- Previously added a blog post resource to the Diagram To Code section:

  - "How to Generate AWS Architecture Diagrams Using Amazon Q CLI and MCP" - A blog post that demonstrates how to use Amazon Q CLI with Model Context Protocol (MCP) to quickly generate AWS architecture diagrams through simple natural language prompts.

- Previously added a blog post resource to the Code Transformation & Modernization section:

  - "Amazon Q: Legacy Java Migration to AWS Graviton Containers" - A blog post that demonstrates how Amazon Q Developer helps modernize Java 8 apps to containerized Java 21 applications optimized for AWS Graviton on Amazon EKS.

- Moved the "Extending AI Capabilities with On-the-Fly Context Using Amazon Q Developer CLI" resource from the Context Management section to the Prompt Engineering section to maintain a clean category structure.

- Removed empty sections that were accidentally added (Context Management and MCP).

- Previously added a GitHub repository resource to the Diagram To Code section:
